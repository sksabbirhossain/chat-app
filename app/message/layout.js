...
const MessageLayout = ({ children }) => {
  ...
  return (
    <div className="mx-auto w-full max-w-5xl sm:pt-4">
      ...
      <aside
        class={`fixed h-screen w-64 -translate-x-full overflow-hidden rounded-lg transition-transform sm:h-[95vh] sm:translate-x-0 ${openSidebar ? "translate-x-0" : ""}`}
      >
      ...
      {/* message contents */}
      <div class="sm:ml-64 sm:px-2">{children}</div>
    </div>
  );
};
